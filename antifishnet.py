...
# This file is part of the liantichess antifishnet client.
...
try:
    ...
    try:
        ...
        print("antifishnet requires pyffish >=0.0.57", file=sys.stderr)
        raise
...
__author__ = "SriMethan"
__license__ = "GPLv3+"

DEFAULT_ENDPOINT = "https://liantichess.herokuapp.com/fishnet/"
STOCKFISH_RELEASES = "https://api.github.com/repos/srimethan/Fairy-Stockfish/releases/latest"
DEFAULT_THREADS = 3
HASH_MIN = 16
HASH_DEFAULT = 256
HASH_MAX = 512
...
def update_self():
    ...
    result = requests.get("https://pypi.org/pypi/antifishnet/json", timeout=HTTP_TIMEOUT).json()
...
def validate_stockfish_command(stockfish_command, conf):
    ...
    required_variants = set([
        "antichess", "losers", "anti_antichess", "antiatomic", "antihouse", "antipawns", "antiplacement", "antihoppelpoppel", "coffee_3check", "coffeerace", "coffeehouse", "coffeehill", "atomic_giveaway_hill", "antishinobi", "antiempire", "antisynochess", "antiorda", "antigrandhouse", "antichak", "anticapablanca", "antishogun", "antihoppelpoppel"])
...
def update_available():
    ...
    if latest_version == __version__:
        logging.info("[antifishnet v%s] Client is up to date", __version__)
        return False
    else:
        logging.info("[antifishnet v%s] Update available on PyPI: %s",
